...
# sensitivity test, all taxa respond like most extreme group 
# (arthropods, LUI factors=0.1)
#
...
climate_scen_name <- c('rcp45_HadGEM2-CC','rcp45_MPI-ESM-MR',
                    'rcp85_HadGEM2-CC','rcp85_MPI-ESM-MR')
...
species_names <- dirs
sp = 'arthropoda' # this will select LUI maps 
...
for( lui in 1:length(lui_scen_names)){
  ...
  for (crop in 1:length(crop_scen_names)){
    ...
    out_string<-paste(gsub('X2080','',lui_scen_names[[lui]]), gsub('rcp','',climate_scen_name[[fut]]),'_LUI_test',sep='')
    outfile<-paste(path,'02_environmental_data/LUI_sensitivity/SA_data',out_string,'.RDS',sep='') 
    print(outfile)
